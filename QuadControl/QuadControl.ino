...
#define RC_YAW_MIN   1330
#define RC_YAW_MAX   1710
#define RC_PIT_MIN   1380
#define RC_PIT_MAX   1700
#define RC_ROL_MIN   1320
#define RC_ROL_MAX   1700
...
void setup()
{
  ...
  for (int i=0; i<4; i++) {
    ...
    pids[PID_YAW_RATE].kP(3); //3
    pids[PID_YAW_RATE].kI(0.1); //1
    pids[PID_YAW_RATE].imax(50); //50

    pids[PID_PITCH_STAB].kP(2);
    pids[PID_ROLL_STAB].kP(2);
    pids[PID_YAW_STAB].kP(0); //6 */

    //QuadControl avec gains de owenson
    // PID Configuration
    pids[PID_PITCH_RATE].kP(0.7);
    pids[PID_PITCH_RATE].kI(0.1);
    pids[PID_PITCH_RATE].imax(10);

    pids[PID_ROLL_RATE].kP(0.7);
    pids[PID_ROLL_RATE].kI(0.1);
    pids[PID_ROLL_RATE].imax(10);

    pids[PID_YAW_RATE].kP(0.7);
    pids[PID_YAW_RATE].kI(0.1);
    pids[PID_YAW_RATE].imax(10);

    pids[PID_PITCH_STAB].kP(2);
    pids[PID_ROLL_STAB].kP(2);
    pids[PID_YAW_STAB].kP(4);
  ...
  void pilotage(){
    ...
    rcthr = channels[2];
    rcyaw = map(channels[3], RC_YAW_MIN, RC_YAW_MAX, -45, 45);
    ...
    // declaration et initialisaion des variables pour le print
    ...
    if(rcthr > RC_THR_MIN + 100) {  // Throttle raised, turn on stablisation.
      ...
      */
      ...
      pitch_stab_output = pids[PID_PITCH_STAB].get_pid((float)rcpit - pitch, 1);
      roll_stab_output = pids[PID_ROLL_STAB].get_pid((float)rcroll - roll, 1);
      yaw_stab_output = pids[PID_YAW_STAB].get_pid((float)wrap_180(yaw_target - yaw), 1);

      //yaw_stab_output = yaw_target;

    int pitch_output =  pids[PID_PITCH_RATE].get_pid(pitch_stab_output - gyroPitch, 1);
    int roll_output =   pids[PID_ROLL_RATE].get_pid(roll_stab_output - gyroRoll, 1);
    int yaw_output =  pids[PID_YAW_RATE].get_pid(yaw_stab_output - gyroYaw, 1);
    ...
    // QUAD CLASSIC
    mot_out_fl = rcthr + roll_output + pitch_output - yaw_output;
    mot_out_bl = rcthr + roll_output - pitch_output + yaw_output;
    mot_out_fr = rcthr - roll_output + pitch_output + yaw_output;
    mot_out_br = rcthr - roll_output - pitch_output - yaw_output;

    mot_out_fl = (long) constrain(mot_out_fl, RC_THR_MIN, RC_THR_MAX);
    mot_out_bl = (long) constrain(mot_out_bl, RC_THR_MIN, RC_THR_MAX);
    mot_out_fr = (long) constrain(mot_out_fr, RC_THR_MIN, RC_THR_MAX);
    mot_out_br = (long) constrain(mot_out_br, RC_THR_MIN, RC_THR_MAX);
  ...
  hal.console->printf("%d, %d, %d, %d, %5.2f, %5.2f, %5.2f, %u, %u, %u, %u, %5.1f, %u, %u, %u, %u, %5.2f, %5.2f, %5.2f, %d, %d, %d \n",
  rcroll,
  rcpit,
  rcthr,
  rcyaw,
